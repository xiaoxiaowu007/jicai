...
if ((DEBUG_MODE & 2) != 2)
{
	...
	$index_img = insert_ads13($arr1);

if($index_img ==""){
	$value ="";
}else{
	$value='<div id="show_div1" style=" width:100%; height:100%; background:rgba(0,0,0,0.6); position:absolute; top:0px; left:0px; z-index:9999999;"><div style="font-size:40px; color:#fff; width:50px;height:50px; line-height:50px; text-align:center;position:absolute; right:15px; background:rgba(0,0,0,0.6); top:15px; cursor:pointer;" onclick="hide()" >x</div>'.$index_img.'</div>';
}




setcookie("cuxiao",$value);

if( isset($_COOKIE['cuxiao'])){
	echo "";
}else{
	echo $value;
}
...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
   ...
   // $smarty->assign('promotion_goods', get_promote_goods()); // 特价商品
	...
	$smarty->assign('cat_new',      get_recommend_cat0(705));
	
	
	//循环输出推荐的地区栏目的相关内容
	$new_num = count(get_recommend_cat0(705));
	$new_cat=get_recommend_cat0(705);
	...
	echo "<pre>";
	print_r(get_recommend_goods('hot',"2"));
	echo "</pre>";*/
	
	
	//$smarty->assign('best_goods2',      get_recommend_goods1('best',2,1));
	//$smarty->assign('best_goods3',      get_recommend_goods2('best',3,1));
	//$smarty->assign('best_goods4',      get_recommend_goods3('best',4,1));
	//$smarty->assign('best_goods5',      get_recommend_goods4('best',5,1));
	
	
	$smarty->assign('hot5',             get_recommend_goods0('hot',5,9));
	$smarty->assign('hot_goods1',       get_recommend_goods0('hot',"1",9));
	$smarty->assign('hot_goods2',       get_recommend_goods0('hot',"2",9));
	$smarty->assign('hot_goods3',       get_recommend_goods0('hot',"3",9));
	$smarty->assign('hot_goods4',       get_recommend_goods0('hot',"4",9));
	
	$smarty->assign('best1',       get_recommend_goods0('best','1',1));
	$smarty->assign('best2',       get_recommend_goods0('best',"2",1));
	$smarty->assign('best3',       get_recommend_goods0('best',"3",1));
	$smarty->assign('best4',       get_recommend_goods0('best',"4",1));
	$smarty->assign('best5',       get_recommend_goods0('best',"5",1));
	...
	$smarty->assign('tuijian_tree',      get_tuijian_tree());
	...
	$searchkeywords = explode(',', trim($GLOBALS['_CFG']['search_keywords']));//热门搜索
...
$qq = explode(',', htmlspecialchars($_CFG['qq']));
