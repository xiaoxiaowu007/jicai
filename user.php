...
if ($action == 'default')
{
	...
	//获取收藏的商品
	$shoucang=get_collection_goods($user_id,5, $pager['start']);
	$shoucang_num=count($shoucang);
	
	$orders = get_user_orders($user_id,4, $pager['start']);
	$orders_num=count($orders);
	
	$smarty->assign('shoucang',        $shoucang);
	$smarty->assign('shoucang_num',        $shoucang_num);
	$smarty->assign('orders_num',        $orders_num);
