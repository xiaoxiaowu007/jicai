...
function insert_ads0($arr)
{
    ...
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link,a.ad_link1,a.ad_link2,a.ad_link3,a.ad_link4, a.ad_code,a.ad_code1,a.ad_code2,a.ad_code3,a.ad_code4, a.ad_name, p.ad_width, ' .
    ...
    foreach ($res AS $row)
    {
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
				...
				$src1 = (strpos($row['ad_code1'], 'http://') === false && strpos($row['ad_code1'], 'https://') === false) ?
                        DATA_DIR . "/afficheimg/$row[ad_code1]" : $row['ad_code1'];
				$src2 = (strpos($row['ad_code2'], 'http://') === false && strpos($row['ad_code2'], 'https://') === false) ?
                        DATA_DIR . "/afficheimg/$row[ad_code2]" : $row['ad_code2'];
				$src3 = (strpos($row['ad_code3'], 'http://') === false && strpos($row['ad_code3'], 'https://') === false) ?
                        DATA_DIR . "/afficheimg/$row[ad_code3]" : $row['ad_code3'];
				$src4 = (strpos($row['ad_code4'], 'http://') === false && strpos($row['ad_code4'], 'https://') === false) ?
                        DATA_DIR . "/afficheimg/$row[ad_code4]" : $row['ad_code4'];
							
							
			    $ads[] = '<div id="fsD1" class="focus" style="position:relative;">
				<div id="D1pic1" class="fPic">
				<div class="fcon" style="display: block;"><a href="'.$row['ad_link'].'" style=" display:block;width:100%; height:497px; background:url('.$src.') no-repeat center center;"><div style="width:100%; height:497px; background:#000"></div></a></div>
				<div class="fcon" style="display: none;"><a href="'.$row['ad_link1'].'" style=" display:block;width:100%; height:497px; background:url('.$src1.') no-repeat center center;"><div style="width:100%; height:497px; background:#000"></div></a></div>
				<div class="fcon" style="display: none;"><a href="'.$row['ad_link2'].'" style=" display:block;width:100%; height:497px; background:url('.$src2.') no-repeat center center;"><div style="width:100%; height:497px; background:#000"></div></a></div>
				<div class="fcon" style="display: none;"><a href="'.$row['ad_link3'].'" style=" display:block;width:100%; height:497px; background:url('.$src3.') no-repeat center center;"><div style="width:100%; height:497px; background:#000"></div></a></div>
				<div class="fcon" style="display: none;"><a href="'.$row['ad_link4'].'" style=" display:block;width:100%; height:497px; background:url('.$src4.') no-repeat center center;"><div style="width:100%; height:497px; background:#000"></div></a></div></div>
               ...
               /* $ads[] = "<div style='height:".$row["ad_height"].";width:".$row["ad_width"].";background:url($src) center center no-repeat;'><a style='display:block;width:100%;height:100%;' href='" .$row["ad_link"]. "' target='_blank'></a></div>";*/
                break;
            case 1: // Flash
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                        DATA_DIR . "/afficheimg/$row[ad_code]" : $row['ad_code'];
    ...
    foreach ($res AS $row)
    {
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                        DATA_DIR . "/afficheimg/$row[ad_code]" : $row['ad_code'];
                $ads[] = "<div style='width:1200px; margin:0 auto;'><a href='affiche.php?ad_id=$row[ad_id]&amp;uri=" .urlencode($row["ad_link"]). "'
...
function insert_ip()
{
    $sql = 'SELECT id from ecs_fangke';
    $bought_notes = $GLOBALS['db']->getAll($sql);
	
	$val=count($bought_notes);
...
function  pei_ads($id){
	...
	'616'=>'181',
	'638'=>'209',
	'496'=>'208','553'=>'198','669'=>'197',
	'642'=>'207','644'=>'206','595'=>'172','667'=>'199',
	'498'=>'205','646'=>'204','610'=>'184','499'=>'203',
	'500'=>'202','666'=>'201','529'=>'200','419'=>'186',
	'592'=>'190','569'=>'191','567'=>'192','699'=>'193',
	'620'=>'194','556'=>'195','622'=>'196','497'=>'164',
	'709'=>'165','708'=>'166','463'=>'167','461'=>'169',
	'459'=>'168','559'=>'170','520'=>'171','652'=>'173',
	'627'=>'174','626'=>'176','623'=>'177','621'=>'178',
	'619'=>'179','617'=>'180','615'=>'182','612'=>'183',
	'554'=>'185','462'=>'187','425'=>'188','557'=>'189',);
