...
if (!$smarty->is_cached('article_cat.dwt', $cache_id))
{
   ...
   /* if ($meta === false || empty($meta))
    ...
    $count  = get_article_sjs_count($cat);
    ...
    if (isset($_REQUEST['keywords']))
    {
        ...
        $count  = get_article_sjs_count($cat, $keywords);
        $pages  = ($count > 0) ? ceil($count / $size) : 1;
        if ($page > $pages)
        {
            $page = $pages;
    ...
    $smarty->assign('artciles_list',    get_cat_articles_sjs($cat, $page, $size ,$keywords));
