...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
	...
	*/
    ...
    $next_article = $db->getRow("SELECT id, ".$name." FROM " .$table. " WHERE id > $article_id  LIMIT 1");
	
    if (!empty($next_article))
    {
        //$next_article['url'] = build_uri('article', array('aid'=>$next_article['id']), $next_article['sjs_name']);
		$next_article['url'] = "article_type.php?id=".$next_article['id'].".&type=sjs";
		
        $smarty->assign('next_article', $next_article);
    }

    $prev_aid = $db->getOne("SELECT max(id) FROM " . $table . " WHERE id < $article_id ");
	
    if (!empty($prev_aid))
    {
        $prev_article = $db->getRow("SELECT id, ".$name." FROM " .$table. " WHERE id = $prev_aid");
        $prev_article['url'] = build_uri('article', array('aid'=>$prev_article['id']), $prev_article['sjs_name']);
...
if(isset($article))
{
	...
	if($_REQUEST['type'] == 'sjs'){
		 $smarty->assign('lanmu_type',     'sjs');
		$smarty->display('article_sjs_info.dwt', $cache_id);
	}else{
		$smarty->display('article_jg_info.dwt', $cache_id);
	}
}
else
{
...
function get_article_info_sjs($article_id)
{
    /* 获得文章的信息 */
    $sql = "SELECT a.* FROM " .$GLOBALS['ecs']->table('article_sjs'). " AS a WHERE a.id = '$article_id' GROUP BY a.id";
...
function get_article_info_jg($article_id)
{
    /* 获得文章的信息 */
    $sql = "SELECT a.* FROM " .$GLOBALS['ecs']->table('article_jg'). " AS a WHERE a.id = '$article_id' GROUP BY a.id";
