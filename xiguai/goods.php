...
$qq = explode(',', htmlspecialchars($_CFG['skype']));
...
/*$id1_arr0 = array_merge($id1_arr1,$id1_1);

if(in_array($,$id1_arr0)){
}*/
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
	...
	if(!$mes[0]['kfqq']){
		...
		$mes1 = $db->getAll($cat_sql1);
		
		
		if(!$mes1[0]['kfqq']){
			
			
			if( $mes[0]['parent_id'] == 0){
				
				$cat_sql3 ="select cat_id from ecs_goods_cat  where goods_id = ".$goods['goods_id'];
		
				$mes3 = $db->getAll($cat_sql3);
				
				foreach($mes3 as $key => $val){
					
					$new_id[] = $val['cat_id']; 
					
				}
				
				echo "djslkjf:".$new_id_str = implode(",",$new_id);
				
				$cat_sql4 ="select parent_id,kfqq from ecs_category  where cat_id in ( ".$new_id_str.") ";
		
				$mes4 = $db->getAll($cat_sql4);
				
				//判断从扩展分类中的栏目id来的kfqq是否存在值
				foreach($mes4 as $key => $val){
					if($val['kfqq']){
						$t_qq = $val['kfqq'];
					}
					$new_id2[] = $val['parent_id']; 
				}
				...
				if(!$t_qq){
					...
					$new_id_str2 = implode(",",$new_id2);
					$cat_sql5 ="select parent_id,kfqq from ecs_category  where cat_id in ( ".$new_id_str2.") ";
					$mes5 = $db->getAll($cat_sql5);
					
					
					//判断从扩展分类中的上一级栏目id来的kfqq是否存在值
					foreach($mes5 as $key => $val){
						if($val['kfqq']){
							$t_qq = $val['kfqq'];
						}
					}
				...
				//如果不在，则根据父级栏目id找寻上一级
				$cat_sql2 ="select parent_id,kfqq from ecs_category  where cat_id =".$mes1[0]['parent_id'];
			
				$mes2 = $db->getAll($cat_sql2);
    ...
    else
    {
		...
		$smarty->assign('people',          get_recommend_sjs('sjs'));                            //获取推荐设计师
		
		
		$smarty->assign('goods_recommend',          get_keywords_recommend($goods_id));          //获取推荐产品
		
		
		$smarty->assign('shouhou',          get_shouhou_article(68));                      //获取产品的售后信息
		
		
		echo "<div style='display:none;'>";
		
		echo get_shouhou_article(68);
...
**/
function get_recommend_sjs($cat)
{
	  $sql = "select id,".$cat."_name,experience,".$cat."_img from  ecs_article_".$cat." LIMIT 3";
