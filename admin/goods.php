...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
	...
	if($cat_id){
		//判断栏目权限是否存在，如果存在的话，则获取栏目id，不存在的话 提示跳转
		if(strpos(',' . $_SESSION['rank_id_goods'] . ',', "goods_g".$cat_id."goods_g") === false){
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
		...
		//print_r($_POST['attr_market_price']);
        ...
        if(isset($_POST['attr_id_list']))
        {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id)
            {
				...
				$attr_img = $url_arr[$key];
				$attr_order = $_POST['attr_order_list'][$key];
				$attr_market_price = $_POST['attr_market_price'][$key];
				
				echo $_POST['attr_market_price'][$key];
                ...
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
						...
						$goods_attr_list[$attr_id][$attr_value]['attr_order'] = $attr_order;
						$goods_attr_list[$attr_id][$attr_value]['attr_market_price'] = $attr_market_price;
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    ...
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price,attr_img,attr_order,attr_market_price)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]','$info[attr_img]','$info[attr_order]','$info[attr_market_price]')";
							
				}
                elseif ($info['sign'] == 'update')
                {
					if($info['attr_img'] == ""){
                    	$sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]',attr_order = '$info[attr_order]',attr_market_price = '$info[attr_market_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
					}else{
						$sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]',attr_img = '$info[attr_img]',attr_order = '$info[attr_order]',attr_market_price = '$info[attr_market_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
					}
                }
                else
                {
