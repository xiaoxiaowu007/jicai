...
/*		print_r($goods_list);
        ...
        $suppliers_list = get_suppliers_list();//获取供货商列表
...
elseif ($_REQUEST['act'] == 'operate_post')
{
	...
	//查询出下订单的产品的id和数量
	$order_sql=" select goods_id,goods_number from ecs_order_goods where order_id = ".$order_id." order by goods_id";
	
	$order_goods0=$GLOBALS['db']->getAll($order_sql);	
	
	$num0 = count($order_goods0);
	...
	if($num0 > 1){
		...
		for($i=0;$i<$num0;$i++){
			if($i != ($num0-1)){
				$goods_id .= $order_goods0[$i]['goods_id'].",";
			}else{
				$goods_id .= $order_goods0[$i]['goods_id'];
			}
		}
		...
		$goods_id  = "=".$order_goods0[0]['goods_id'];
		
	}
	
	//根据获取的商品id 获取现有商品的销售数量
	$goods_sales_sql="select sales_num,goods_id from ecs_goods where goods_id ".$goods_id." order by goods_id";	
	
	$goods_id_num=$GLOBALS['db']->getAll($goods_sales_sql);
	
	$num1=count($goods_id_num);
	...
	if(($num1 == $num0)&& $num1 >0){
		...
		for( $i=0;$i < $num1; $i++){
			$goods_info[$i]['sales_num']=$goods_id_num[$i]['sales_num']+$order_goods0[$i]['goods_number'];
			$goods_info[$i]['goods_id'] =$goods_id_num[$i]['goods_id'];
		}
...
/*	
    ...
    if ('confirm' == $operation)
    {
		...
		if($goods_info){
			...
			for($i=0;$i<$num1;$i++){	
				$goods_up_sql=" update ecs_goods set sales_num =".$goods_info[$i]['sales_num']."  where goods_id=".$goods_info[$i]['goods_id'];
				
				echo $goods_up_sql;
				
				echo "<hr/>";
				
				$GLOBALS['db']->query($goods_up_sql);
			}
		}
    ...
    elseif ('pay' == $operation)
    {
        ...
        /* 标记订单为已确认、已付款，更新付款时间和已支付金额，如果是到店付款，同时修改订单为“收货确认” */
