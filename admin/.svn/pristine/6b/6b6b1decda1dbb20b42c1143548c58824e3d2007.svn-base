 ...
 * @param   string  $rank_id      用于判断栏目权限
 * @return  void
 */
function set_admin_session($user_id, $username, $action_list, $last_time,$rank_id,$rank_id_goods)
{
	...
	$_SESSION['rank_id_goods']  = $rank_id_goods;
...
function admin_priv($priv_str, $msg_type = '' , $msg_output = true)
{
    ...
    if (strpos($_SESSION['action_list'],'all') !== false)
    {
        return true;
    }
	
//	echo $_SESSION['action_list']."<br/>";
//	echo $priv_str."<br/>";
...
/**
 * 判断管理员对某一个文章栏目的权限是否有权限。
 *
 * 根据当前对应的action_code，然后再和用户session里面的action_list做匹配，以此来决定是否可以继续执行。
 * @param     int       $cat_id    操作对应的priv_str
 * @param     string    $msg_type       返回的类型
 * @return true/false
 */
function admin_cpriv($cat_id)
{
    global $_LANG;
    ...
    if (strpos(',' . $_SESSION['action_list'] . ',', "goods_c".$cat_id) === false)
    {
        $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
...
{
	if(preg_match('/,*'.$authz.',*/', $_SESSION['action_list']) || strpos($_SESSION['action_list'] ,"all") !== false ){
		return TRUE;
	}else{
		return false;
	}
