...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values,v.attr_order, v.attr_value, v.attr_price,v.attr_market_price ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        {
            $html .= '<input name="attr_value_list[]" type="text" value="' .htmlspecialchars($val['attr_value']). '" size="20" /> ';
        }
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" /><input type="file" style="width:180px;" name ="attr_img_list[]"/>排序：<input style="width:40px;" value="' . $val['attr_order'] . '" type="text" name ="attr_order_list[]"/>市场价格：<input style="width:40px;" value="' . $val['attr_market_price'] . '" type="text" name ="attr_market_price[]"/>' :'<input type="hidden" name="attr_price_list[]" value="0" /><input type="file" style="display:none;" name="attr_img_list[]"/><input type="hidden" name ="attr_order_list[]"/><input type="hidden" name ="attr_market_price[]"/>';
        $html .= '</td></tr>';
    }
...
//	echo "</pre>";
	...
	$rank_id_attr = $_SESSION['rank_id_goods'];
	
	
		if(strpos($rank_id_attr, 'all') === false ){
			$rank_id_arr = explode("goods_g",$rank_id_attr);
			...
			$rank_new_attr = implode(",",$new_rank);
    ...
    if ($result === false)
    {
     ...
     $filter['cat_id']  = strpos($_REQUEST['cat_id'],',') !== false ? $rank_new_attr : intval($_REQUEST['cat_id']);
		
		
		if(empty($_REQUEST['cat_id']) === false){
			
			if(strpos($rank_new_attr,$_REQUEST['cat_id'])  === flase ){
        ...
        if($filter['cat_id'] == ""){
        	...
        	$where = " and g.cat_id in(".$filter['cat_id'].")";
        }
		...
		//还有一个条件就是 通过扩展id查找商品id号
		if( strpos($_REQUEST['cat_id'],',') === false && intval($_REQUEST['cat_id']) != "" ){
			
			$sql00 = "SELECT goods_id FROM " .$GLOBALS['ecs']->table('goods_cat'). " where cat_id in (".$filter['cat_id'].") ";
       		$cat_pro_id = $GLOBALS['db']->getAll($sql00);
			
			$cat_pro_num = count($cat_pro_id);
			
			if( $cat_pro_num > 0 ){
				
				foreach( $cat_pro_id as $key => $val ){
				
						$cat_pro_arr[] = $val['goods_id'] ;
				
				}
				
				$cat_pro_str = implode(",",$cat_pro_arr);
				
				$where .= " or g.goods_id in (".$cat_pro_str.")";
...
//		echo "</pre>";
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn,click_count,shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
