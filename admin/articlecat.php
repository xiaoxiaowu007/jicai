...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	$pre_fix = "article_a";//前缀
	
	$rank_num  = count($rank_id);
	
	$act_id_str = " user_id in (";
	for($i =0 ;$i< $rank_num ;$i++){
		if($i == ($rank_num-1) ){
			$act_id_str .= $rank_id[$i].")";
		}else{
			$act_id_str .= $rank_id[$i].",";
		}
	}
	...
	//查询用户信息中的 权限表格
	$user_sql = "select rank_id,user_id from ".$ecs->table('admin_user').$where." order by add_time asc";
    ...
    $db->query($sql);
	 ...
	 $cat_id = $db->insert_id();
		...
		foreach($user_res as $k => $val){
			$user_new_info[$k]['rank_id'] = $val['rank_id'].",".$pre_fix.$cat_id.$pre_fix;
			$user_new_info[$k]['user_id']     = $val['user_id'];
		}
		
		//更新数据库 
		$update_user_sql = "update ecs_admin_user set rank_id = case user_id";
		
		//获取更新数据队列
		foreach($user_new_info as $key => $val){
			$key = $key+1;
			 $update_user_sql  .= " when ".$val['user_id']." then '".$val['rank_id']."'";
		}
...
elseif ($_REQUEST['act'] == 'edit')
{
	...
	foreach($userlist as $key => $val){
		if(strpos($val['rank_id'],"article_a".$cat_id) !== false || strpos("0".$val['rank_id'],'all')){
			$userlist[$key]['rank_id_on'] = '1';
		}else{
			$userlist[$key]['rank_id_on'] = '0';
		}
	}
...
elseif ($_REQUEST['act'] == 'update')
{
	...
	//查询用户信息中的 权限表格
	$user_sql = "select rank_id,user_id from ".$ecs->table('admin_user')." order by add_time asc";
	
	$user_res = $db->getAll($user_sql);
	
	$cat_id = $_POST['id'];
	
	//判断会员id是否在字符串中，不在的话，把权限表中原来的对应的栏目权限去掉
	foreach($user_res as $k => $val){
		
		if(strpos($act_id_str,$val['user_id']) === false){
			$val['rank_id'] = str_replace(",".$pre_fix.$cat_id.$pre_fix,"",$val['rank_id']);
		}
	...
	//echo str_replace("world","Shanghai","Hello world!");
		...
		foreach($user_res as $k => $val){
			...
			//判断字符串中是否含有用户id，如果有则进行更新，没有的去除原来的分类权限
			if(strpos($act_id_str,$val['user_id']) !== false){
				
				//判断分类权限中是否已经含有该权限，如果含有不予以处理，没有的话,进行添加
				if(strpos($val['rank_id'],",".$pre_fix.$cat_id.$pre_fix) === false){
					$user_new_info[$k]['rank_id'] = $val['rank_id'].",".$pre_fix.$cat_id.$pre_fix;
					$user_new_info[$k]['user_id'] = $val['user_id'];
				}else{
					$user_new_info[$k]['rank_id'] = $val['rank_id'];
					$user_new_info[$k]['user_id'] = $val['user_id'];
				}
				
			}else{
				$user_new_info[$k]['rank_id'] = str_replace(",".$pre_fix.$cat_id.$pre_fix,"",$val['rank_id']);
				$user_new_info[$k]['user_id'] = $val['user_id'];
			}
//			$user_new_info[$k]['rank_id'] = $val['rank_id'].",".$pre_fix.$cat_id.$pre_fix;
//			$user_new_info[$k]['user_id']     = $val['user_id'];
...
function user_list()
{
    $res = $GLOBALS['db']->getAll("select user_id,user_name,rank_id FROM ". $GLOBALS['ecs']->table("admin_user")." order by add_time asc");
