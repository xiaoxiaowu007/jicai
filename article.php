...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
   ...
   /* $next_article = $db->getRow("SELECT article_id, title FROM " .$table. " WHERE article_id > $article_id AND cat_id=$article[cat_id] AND is_open=1 LIMIT 1");
 ...
 * @return  array
 */
function get_article_info0($article_id)
{
    /* 获得文章的信息 */
    $sql = "SELECT * ".
            "FROM " .$GLOBALS['ecs']->table('ssj')." WHERE id = '$article_id' ";
