...
function get_categories($cat_id = 0)
{
	...
	return array('parent_id'=>$parent_id,'cate'=>$cat_arr);
}
...
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 limit 13";
	
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC  limit 13";
...
function get_child_tree0($tree_id)
{
    $three_arr = array();
    $sql = 'SELECT cat_id,cat_name,is_show FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 order by sort_order";
...
function get_tuijian_tree()
{
	...
	$sql0 = 'SELECT cat_id,cat_name FROM ' . $GLOBALS['ecs']->table('category')." where cat_id ".$cat_id;
	$cat_res0 = $GLOBALS['db']->getAll($sql0);
...
function get_recommend_goods($type = '', $cats = '')
{
	...
	if($cats){
	$cat_sql="select cat_id from ecs_category where cat_id =".$cats." or parent_id = ".$cats;
	$cat_res = $GLOBALS['db']->getAll($cat_sql);
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
			if($num >　0){
				$sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
				   ...
				   ' WHERE g.is_on_sale = 1 and g.cat_id '.$cat_id.' AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1)'.
		...
		$type_merge = array_merge($type_array[$type]);
...
function get_recommend_goods1($type = '', $cats = '')
{
    if (!in_array($type, array('best', 'new', 'hot')))
    {
        return array();
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $data = read_static_cache('recommend_goods1');
        if ($data === false)
        {
			if($num >　0){
				$sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
            ...
            write_static_cache('recommend_goods1', $goods_data);
        }
        else
        {
            $goods_data = $data;
...
function get_recommend_goods0($type = '', $cats = '',$limit = '')
{
    if (!in_array($type, array('best', 'new', 'hot')))
    {
        return array();
...
/*	if($type == 'hot'){
	...
	if($cats){
		...
		//获取三级栏目
		$sql00 =  'SELECT cat_id from ecs_category where parent_id '.$cat_id;
		$result00 = $GLOBALS['db']->getAll($sql00);
		
		$num00=count($result00);
		
		if( $num00 > 0 ){
			
			$j=1;
			$cat_id00="in (";
			foreach($result00 as $val0){
	...
	$sql =  'SELECT goods_id,goods_name,original_img, market_price, shop_price,goods_brief from ecs_goods where cat_id '.$lanm." and is_".$type."=1 and is_delete = 0 order by sort_order asc limit ".$limit;
...
function get_recommend_goods12($type = '', $cats = '',$limit = '')
{
    if (!in_array($type, array('best', 'new', 'hot')))
    {
        return array();
	...
	if($cats){
		...
		$sql_cat0 	 =  'SELECT goods_id from ecs_goods_cat where cat_id '.$lanm;
	
	
		$result_cat0 =  $GLOBALS['db']->getAll($sql_cat0);
		
		
		$num = count($result_cat0);
		
		if( $num > 0){
			foreach($result_cat0 as $key => $val){
			$result_cat_arr[] = $val['goods_id'];
			}
			
			$result_cat_str = implode(",",$result_cat_arr);
	
			if($result_cat_str){
				
				$or_where = " or ( goods_id in (".$result_cat_str.") and is_".$type."= 1 )";
	...
	$sql =  'SELECT goods_id,goods_name,original_img, market_price, shop_price,goods_brief from ecs_goods where cat_id '.$lanm." and is_".$type."=1 and is_delete = 0 ".$or_where." order by sort_order asc limit ".$limit;
...
function get_recommend_goods2($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $type_goods['new'] = array();
        $type_goods['hot'] = array();
        $data = read_static_cache('recommend_goods2');
        if ($data === false)
        {
			if($num >　0){
				$sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
            ...
            write_static_cache('recommend_goods2', $goods_data);
        }
        else
        {
            $goods_data = $data;
...
function get_recommend_goods3($type = '', $cats = '')
{
    if (!in_array($type, array('best', 'new', 'hot')))
    {
        return array();
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $type_goods['new'] = array();
        $type_goods['hot'] = array();
        $data = read_static_cache('recommend_goods3');
        if ($data === false)
        {
			if($num >　0){
				$sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
            ...
            write_static_cache('recommend_goods3', $goods_data);
        }
        else
        {
            $goods_data = $data;
...
function get_recommend_goods4($type = '', $cats = '')
{
    if (!in_array($type, array('best', 'new', 'hot')))
    {
        return array();
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $type_goods['new'] = array();
        $type_goods['hot'] = array();
        $data = read_static_cache('recommend_goods4');
        if ($data === false)
        {
			if($num >　0){
				$sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
            ...
            write_static_cache('recommend_goods4', $goods_data);
        }
        else
        {
            $goods_data = $data;
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                "g.goods_attr_id, g.attr_value, g.attr_price,g.attr_img,g.attr_market_price " .
            'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
            "WHERE g.goods_id = '$goods_id' " .
            'ORDER BY g.attr_order asc,g.attr_price asc';
    $res = $GLOBALS['db']->getAll($sql);
    ...
    foreach ($res AS $row)
    {
        ...
        else
        {
            ...
            $arr['spe'][$row['attr_id']]['values'][] = array(
														...
														'img'           => $row['attr_img'],
														'market_price'           => $row['attr_market_price']);
...
function favourable_info($act_id)
{
	...
	echo "<hr/>";*/
	 ...
	 $new_sql = "SELECT goods_id,goods_name,goods_brief,original_img,shop_price FROM " . $GLOBALS['ecs']->table('goods') .
            " WHERE goods_id ".$new_row;
    $new_row = $GLOBALS['db']->getAll($new_sql);
 ...
 * @return      array
 */
function get_recommend_cat()
{

	$sql = "SELECT cat_id FROM " .$GLOBALS['ecs']->table('cat_recommend'). " WHERE recommend_type = 2";
	...
	$sql0 = "SELECT cat_name FROM " .$GLOBALS['ecs']->table('category'). " WHERE cat_id ".$cat_id;
	
	
	$cat_name=$GLOBALS['db']->getAll($sql0);
 ...
 * @return      array
 */
function get_recommend_cat0($parent_id)
{
	
	//获取推荐表中，推荐类型是3的，并且属于按地区分类的子类中的；
	$sql = "select b.cat_id,a.cat_name from ".$GLOBALS['ecs']->table('cat_recommend')." as b left join ".$GLOBALS['ecs']->table('category')." as a on a.cat_id = b.cat_id where a.parent_id = ".$parent_id." and b.recommend_type = 3 order by sort_order asc";
 ...
 * @return      array
 */
function get_area_name($name)
{	
	
	$sql="SELECT goods_id,goods_name,original_img,market_price,shop_price,goods_brief FROM " .$GLOBALS['ecs']->table('goods'). " WHERE keywords like '%".$name."%' and is_new = 1 order by  sort_order asc limit 8";
...
function get_sjs_info($cat)
{
    $sql = "SELECT id,".$cat."_name,".$cat."_img,".$cat."_brief,experience,left_img,rec_img FROM " .$GLOBALS['ecs']->table('article_'.$cat). "   where is_open=1 LIMIT 1";
    $return_array = $GLOBALS['db']->getRow($sql); 
    return $return_array;
}
 ...
 * @return      array
 */
function get_keywords_recommend($goods_id)
{
	
	$sql0="SELECT keywords from ".$GLOBALS['ecs']->table('goods'). "where goods_id =".$goods_id;
	
	$keys_res=$GLOBALS['db']->getOne($sql0);
	
	$keys_arr=array();
	
	$keys_arr = explode(" ",$keys_res);
	
	$keys_num = count($keys_arr);
	...
	if($keys_num > 1){
		...
		for($i=0;$i < $keys_num; $i++){
			if($i < ($keys_num-1)){
				$tiaojian .= " keywords  like '%".$keys_arr[$i]."%' or";
			}else{
				$tiaojian .= " keywords like '%".$keys_arr[$i]."%'";
			}
		} 
					
	}else{
		if($keys_num == 1){
			$tiaojian .= " keywords like '%".$keys_arr[0]."%'";
		}
	}
	
    $sql = "SELECT goods_id,market_price,shop_price,goods_name,original_img FROM " .$GLOBALS['ecs']->table('goods').$tiaojian." LIMIT 5";
	
	
    $return_array = $GLOBALS['db']->getAll($sql); 
 ...
 * @return      array
 */
function get_shouhou_article($article_id)
{
	
	
	if($article_id){
		$sql = "select content from ecs_article where article_id = ".$article_id;
...
function get_p_num()
{
	...
	$sql = "select count(*) from ecs_fangke";
