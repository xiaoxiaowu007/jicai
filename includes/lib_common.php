...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
		...
		case 'article0':
            ...
            {
                $uri = $rewrite ? 'article-' . $aid : 'article0.php?id=' . $aid;
            }
 ...
 * $cat_id arr
 * */
function article_cat_list_dan($cat_id){
	
	$cat_num = count($cat_id);
	
	if( $cat_num > 0 ){
		if($cat_num > 1){
			$tiaojian = " where cat_id in (";
			for($j = 0;$j < $cat_num; $j++){
				if($j ==  $cat_num -1){
					$tiaojian .= $cat_id[$j];
				}else{
    ...
    $sql = "SELECT cat_name,cat_id " .
                " FROM " . $GLOBALS['ecs']->table('article_cat').$tiaojian;

    $art_cat_arr = $GLOBALS['db']->getAll($sql);
	
	$quan_num = count($art_cat_arr);
	
	foreach($art_cat_arr as $val){
			$art_cat_arr0 .= "<option value='".$val['cat_id']."'>".$val['cat_name']."</option>";
